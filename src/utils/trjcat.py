...
def run(args):
    ...
    for step in range(1, args.num_of_step + 1):
        ...
        subprocess.run(cmd, shell=True)
        LOGGER.info(f"prd_{step}_skip{args.skip}_rmmol.xtc generated")

    c_cmd = "c\n" * 100
    trj_files = [
        f"prd_{step}_skip{args.skip}_rmmol.xtc"
        for step in range(1, args.num_of_step + 1)
    ]
    ...
    subprocess.run(cmd, shell=True)
    LOGGER.info(f"prd_all_skip{args.skip}_rmmol.xtc generated")
