...
Large standard-matte atomic layers use compact native spheres and split cylinders with ``params.atomicMode=auto``. All selected atoms and loaded states are retained. Native atoms use PyMOL lighting/transparency; custom GGX and ambient occlusion apply to mesh layers. Use ``params={"atomicMode": "mesh"}`` from Python to retain full custom shading for every atom. Custom materials, clipping, lighting, and other effects keep the mesh renderer.
